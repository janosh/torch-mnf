...
class MNFLinear(nn.Module):
    ...
    def sample_z(self, batch_size):
        ...
        if not self.use_z:
            return torch.ones([batch_size, self.n_in]), log_dets

        q0_mean = self.q0_mean.repeat(batch_size, 1)
        epsilon = torch.randn([batch_size, self.n_in])
        q0_var = self.q0_log_var.exp()
        z_samples = q0_mean + q0_var ** 0.5 * epsilon
    ...
    def forward(self, x):
        ...
        std_W = torch.clamp(self.log_std_W.exp(), 0, self.max_std)
